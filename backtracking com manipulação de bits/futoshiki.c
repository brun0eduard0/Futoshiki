...
char **readmat(int *n){
	...
	printf("n - %d\n", *n);
	tamanho = (*n);
	rows = (int*) calloc(tam, sizeof(int));
	cols = (int*) calloc(tam, sizeof(int));
...
void solveFutoshiki(char **mat, int row, int col){
	...
	if(mat[row][col]!='0'){			// avança para o próximo quadrante caso hava alguma valor na posição.
		...
		int control = 1;
		int positionRow = (int) ((row + 1) / 2);
		int positionCol = (int) ((col + 1) / 2);
		for(i = 1 ; i <= tam; i= i + 2){
			int mask = 1 << control;			
			if (((rows[positionRow] & mask) == 0) && ((cols[positionCol] & mask) == 0)&&(isValid(mat, row, col, control))){
				mat[row][col] = (char)control+'0';
				rows[positionRow] |= mask;
				cols[positionCol] |= mask;
				if(col < tam-1){
					solveFutoshiki(mat, row, col + 2);
				}
				if(col==tam-1){
